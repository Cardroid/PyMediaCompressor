...
processer_name = "automatic media compressor by cardroid"
is_force = False

input_filepath = os.path.realpath(sys.argv[2])

dirpath = os.path.realpath("out")
...
if is_only_audio:
    ...
    if height > 1440:
        ffmpeg_global_args["vf"] = "scale=-1:1440"

output_filepath = f"{os.path.join(dirpath, os.path.splitext(os.path.basename(input_filepath))[0])}.{ext}"
